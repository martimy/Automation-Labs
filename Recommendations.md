...
├── .venv/                      # Unified virtual environment for the entire course
...
├── .gitignore                  # Updated to ignore .venv/ and lab-specific clab directories
...
2.  **Unified Environment Management:**
    *   By placing `.venv/` at the root, students only need to run `source ~/labs/.venv/bin/activate` regardless of which lab module they are working on.
