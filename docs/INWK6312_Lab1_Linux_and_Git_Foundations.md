...
Target topology: `ns-hostA (10.10.1.2/24) -- ns-router -- ns-hostB (10.10.2.2/24)`.
...
2. Edit the `build-topology.sh` and `teardown-topology.sh` files to add a third host named `ns-hostC`. Connect `ns-hostC` to `ns-router` on a new subnet `10.10.3.0/24`. Update the nameservers array, add the new `veth` pair, and ensure the routing logic and forwarding are correctly extended.
...
7. Merge the feature branch into `main`. Since `main` has not changed, this will be a fast forward merge.
...
5. Repeat step 2, but use `10.10.21.0/24` instead. Commit this change.
